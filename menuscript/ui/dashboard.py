        ...
        try:
            ...
            if result and 'error' not in result:
                ...
                if tool == 'nmap':
                    ...
                    lines.append(f"  • {shares_added} SMB share(s) discovered")
                    if files_added > 0:
                        lines.append(f"  • {files_added} file(s) enumerated")
                    if findings_added > 0:
                        lines.append(click.style(f"  • {findings_added} security finding(s) detected", fg='red', bold=True))
...
def _show_dashboard_menu(engagement_id: int):
    ...
    click.echo("                " + click.style("[8]", fg='cyan') + "  📁 SMB Shares     - View enumerated SMB shares")
    click.echo()
    ...
    click.echo("    " + click.style("[r]", fg='magenta', bold=True) + " or " +
               click.style("[9]", fg='magenta') + "  📄 Manage Reports  - View, generate, or delete reports")
    ...
    try:
        ...
        if choice_num == 1:
            ...
            from menuscript.ui.interactive import view_smb_shares
